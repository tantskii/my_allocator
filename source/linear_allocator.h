...
private:
    ...
    PointerHolder<T> m_pointer_holder;
};
...
T* LinearAllocator<T, Capacity>::allocate(std::size_t n) {
    ...
    if (m_start == nullptr && n <= Capacity) {
        ...
        m_pointer_holder.reserve(Capacity);
        m_pointer_holder.addRange(m_start, n);
        m_used  = m_start + n;
    ...
    // в последющие вызовы используем уже выделенную память
        ...
        m_pointer_holder.addRange(m_used, n);
        m_used += n;
        return m_used - n;
    }
...
void LinearAllocator<T, Capacity>::deallocate(T* p, size_t n) {
    m_pointer_holder.removeRange(p, n);
    
    if (m_pointer_holder.empty()) {
        m_used = m_start;
    }
}
